...
- Optional `effort` (`low`|`medium`|`high`|`xhigh`|`max`) and `model` (e.g. `gpt-5.5`) tune the Codex reasoning tier. Omit them to use the default: **`high`** for both PRs and files (high reasoning on a large file is ~30 min / ~700k tok — pass `effort: 'medium'` or `'low'` when the user wants a faster, shallower pass). Invalid values fall back to the default.
...
Notes:
- Codex runs as `gpt-5.5`; reasoning effort defaults to `high`, overridable via the `effort` arg.
...
- The Bash permissions the Codex subagent needs are shipped in this plugin's `settings.json` and applied when the plugin is enabled, so the auto-mode safety classifier doesn't block it: `Bash(codex exec:*)`, `Bash(mktemp:*)`, `Bash(rm -f:*)`, `Bash(git rev-parse:*)`, `Bash(mkdir -p:*)`, plus `Bash(find:*)`, `Bash(grep:*)`, `Bash(cat:*)` which the generated session-capture bash also runs, and the scoped `gh` prefixes `Bash(gh pr view:*)`, `Bash(gh pr diff:*)`, `Bash(gh pr comment:*)`, `Bash(gh repo view:*)`, `Bash(gh api:*)` — deliberately NOT a blanket `Bash(gh:*)`: these grants apply to every session while the plugin is enabled, so they stay as narrow as the workflow allows (no `gh pr merge`, `gh repo delete`, `gh secret`, …). If `codex exec` is still blocked (e.g. the plugin isn't enabled), add these to `.claude/settings.local.json` and re-run.
